...
public class FloatArray2DSIFT {
	...
	private float[] createDescriptor(float[] c, int o, float octave_sigma, float orientation) {
		...
		for (int y = FEATURE_DESCRIPTOR_WIDTH - 1; y >= 0; --y) {
			float ys = ((float) y - 2.0f * (float) FEATURE_DESCRIPTOR_SIZE + 0.5f) * octave_sigma; // !<
																									// scale
																									// y
																									// around
																									// 0,0
			for (int x = FEATURE_DESCRIPTOR_WIDTH - 1; x >= 0; --x) {
				float xs = ((float) x - 2.0f * (float) FEATURE_DESCRIPTOR_SIZE + 0.5f) * octave_sigma; // !<
																										// scale
																										// x
																										// around
																										// 0,0
				...
				int yg = Filter.flipInRange((int) (Math.round(yr + c[1])), gradients[0].height);

				// translate xs to sample x position in the gradient image
				int xg = Filter.flipInRange((int) (Math.round(xr + c[0])), gradients[0].width);
	 ...
	 * 
	 * @param fs1
	 *            feature collection from set 1 sorted by decreasing size
	 * @param fs2
	 *            feature collection from set 2 sorted by decreasing size
	 * @param max_sd
	 *            maximal difference in size (ratio max/min)
	 * @return matches
	 * 
	 *         TODO implement the spatial constraints
	 */
	public static int createMatches(List<Feature> fs1, List<Feature> fs2, float max_sd) {
		...
		for (Feature f1 : fs1) {
			...
			if (max_sd < Float.MAX_VALUE) {
				while (s > 1) {
					...
					if (f2.scale / f1.scale < min_sd)
						last = Math.max(0, last - s);
					else
						last = Math.min(size_1, last + s);
					f2 = fs2.get(first);
					if (f2.scale / f1.scale < max_sd)
						first = Math.max(0, first - s);
					else
						first = Math.min(size_1, first + s);
					s = s / 2 + s % 2;
				}
			}
